    ...
    @foreach ($uploadingMessages as $uploadingMessage)
        <div class="flex flex-row items-center justify-end">
            <div style="flex: 0 0 30px"
                 class="flex justify-content-center align-items-center relative loader w-[30px] h-[30px] mr-2"></div>
            ...
            <div style="flex: 0 1 auto" class="relative flex flex-col content-end">
                ...
                @php
                    $uploadingImages = TemporaryUploadedFile::unserializeFromLivewireRequest($uploadingMessage['temporary_files']);
                @endphp
                ...
                @if (!empty($uploadingImages))
                    ...
                    <div class="relative"
                         style="width: {{200 + (count($displayUploadingImages) - 1) * 25}}px; height: {{200 + (count($displayUploadingImages) - 1) * 25}}px;">
                        @foreach ($displayUploadingImages as $index => $displayImage)
                            <div class="absolute bg-gray-300 w-[200px] h-[200px] rounded-xl"
                                 style="bottom: {{$index * 25}}px; left: {{$index * 25}}px; z-index: {{5 - $index}}">
                                <img src="{{ $displayImage->temporaryUrl() }}"
